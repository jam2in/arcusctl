...
func List() error {
	...
	fmt.Fprintln(w, "SERVICECODE\tVERSION\tEDITION\tNODES\tGROUPS\tDEPLOYED_AT")
	...
	for _, serviceCode := range serviceCodes {
		...
		if topo, err := store.LoadClusterTopology(serviceCode); err == nil {
			...
			if e, err := topo.Edition(); err == nil {
				edition = string(e)
				if e == topology.EnterpriseEdition {
					groups = strconv.Itoa(topo.GroupCount())
				}
			}
		}

		fmt.Fprintf(w, "%s\t%s\t%s\t%d\t%s\t%s\n",
			serviceCode, meta.Version, edition, servers, groups,
